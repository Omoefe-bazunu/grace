...
//This is for the language feature for sermons uploaded
const LANGUAGES = [
  { code: 'en', label: 'English' },
  { code: 'fr', label: 'French' },
  { code: 'es', label: 'Spanish' },
  { code: 'ur', label: 'Ephẹrẹ rẹ Urhobo' },
  { code: 'yo', label: 'Yoruba' },
  { code: 'ig', label: 'Igbo' },
  { code: 'ha', label: 'Hausa' },
  { code: 'is', label: 'Ẹp hẹrẹ Isoko' },
  { code: 'its', label: 'Uvun Itṣẹkiri' },
  { code: 'ij', label: 'Ijaw' },
];
...
export default function UploadScreen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // For sermons, nest title+content under translations[lang]
      let payload = { ...formData, uploadedBy: user?.email || 'admin' };
      if (type === 'sermon') {
        const { title, content, language, ...rest } = payload;
        payload = {
          ...rest,
          translations: {
            [lang]: { title, content },
      ...
      // Only trigger TTS for English sermons
      if (type === 'sermon' && lang === 'en' && formData.content?.trim()) {
          ...
          >
            {LANGUAGES.map((lang) => (
              <TouchableOpacity
                ...
                style={[
                  styles.categoryChip,
                  (formData.language || 'en') === lang.code &&
