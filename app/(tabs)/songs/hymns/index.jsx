...
// Cache keyed by language so switching language never returns stale data
const combinedCache = {
  store: {}, // { [language]: { data, timestamp } }
  get: function (language) {
    const entry = this.store[language];
    if (!entry) return null;
    // Cache expiry of 5 minutes
    return Date.now() - entry.timestamp < 300000 ? entry.data : null;
  },
  set: function (language, hymnList, psalmList) {
...
export default function HymnsScreen() {
  ...
  const loadItems = useCallback(async () => {
    try {
      setLoading(true);
      const cached = combinedCache.get(currentLanguage);
      if (cached) {
        setItems(cached);
        return;
      }
      ...
      combinedCache.set(currentLanguage, hymns, psalms);
      setItems(combinedCache.get(currentLanguage));
    } catch (error) {
